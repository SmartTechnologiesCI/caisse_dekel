table 70026 "Transfert Stock"
{
    ...
    fields
    {
        field(1; "No."; Code[50])
        {
            ...
            begin
                if rec."No." <> xRec."No." then begin
                    PStock.Get();
                    NoSeriesMgt.TestManual(PStock."N° Transfert");
                    rec."No. Series" := '';
        ...
        field(19; "Location CodeDest"; Code[30])
        {
            Caption = 'Code magasin Destinantion';
            DataClassification = ToBeClassified;
            TableRelation = Location.Code;
        }

        field(20; "curr Carton Mag dest"; Integer)
        {
            Caption = 'Cartons actuels du magasin';
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(21; "curr Quantity Mag Dest"; Decimal)
        {
    ...
    begin
        if "No." = '' then begin
            PStock.get();
            PStock.TestField("N° Transfert");
            NoSeriesMgt.InitSeries(PStock."N° Transfert", xRec."No. Series", 0D, "No.", "No. Series");
        end;
    end;
