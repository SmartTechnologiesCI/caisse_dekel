tableextension 70007 "Achat Activity Role Center Ext" extends 50008
{
    fields
    {
        ...
        {
            FieldClass = FlowField;
            CalcFormula = count("Purchase Header" where("Statut paiement" = const("Paid"), "Document Type" = const(Order)));
        }
        field(10; "Contrats Non payés"; Integer)
        {
            FieldClass = FlowField;
            CalcFormula = count("Purchase Header" where("Statut paiement" = const("Unpaid"), Avance = filter('<=0'), "Document Type" = const(Order)));
        }
